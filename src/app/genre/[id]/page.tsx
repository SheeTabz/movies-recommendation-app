...
export default function GenrePage() {
  ...
  const genreId = params?.id ? parseInt(params.id as string) : undefined;
  const genreName = searchParams?.get('name') || 'Movies';
  
  const { movies, loading, error } = useDiscoverMovies(genreId);
    ...
    return (
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="pt-20 text-center py-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">{genreName} Movies</h2>
          <p className="text-gray-400">Unable to load {genreName.toLowerCase()} movies</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-20 px-4 md:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">{genreName} Movies</h2>
